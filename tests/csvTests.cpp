...
TEST_CASE("Testing parsing a good file", "[csvParser]") {
	std::vector<std::tuple<int, unsigned int, float, double, std::string, bool>> contents;
	std::string file = "tests/testData/good.csv";

	bool result = parseFile<int, unsigned int, float, double, std::string, bool>(file, contents);
